...
func renderDockerCompose(r *GenerateResult) string {
	...
	if r.IncludeRedis {
		...
		if r.RedisPassword != "" {
			fmt.Fprintf(&b, "    command: redis-server --requirepass ${REDIS_PASSWORD:-%s} --maxmemory 128mb --maxmemory-policy allkeys-lru\n", r.RedisPassword)
		} else {
	...
	if r.IncludePostgres {
		...
		if pgPass == "" {
			pgPass = "csar_dev" //nolint:gosec // G101: dev-only default, not a real credential
		}
		b.WriteString("  postgres:\n")
		...
		fmt.Fprintf(&b, "      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-%s}\n", pgPass)
		b.WriteString("    ports:\n")
